...
void SetPara_DFT()
{
  ...
  ****************************************************/
  ...
  /* added by Parq May/2015 */
  if (alpha_calc)  {

    wanA = WhatSpecies[target_atom];

    if (target_l[0]<0 || target_l[0]>Spe_MaxL_Basis[wanA]) {

      if (myid==Host_ID){
        printf("Wrong number for scf.alpha.l\n");
      }
      MPI_Finalize();
      exit(1);
    }

    if (target_l[1]>Spe_MaxL_Basis[wanA]) target_l[1] = -1;
  }
  /* until here */
